...
curl 'https://dnsdumpster.com/' -H 'User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:45.0) Gecko/20100101 Firefox/45.0'--data "csrfmiddlewaretoken=u&targetip=$target" --insecure | grep "md-4" | awk '{print $2}' | cut -d '>' -f2 | grep -F . | uniq | cut -d '<' -f1 | sed 's/\.$//' | grep "$target" >> /tmp/onlineFoundSubdomains
